...
function install_package(){
    ...
    if(!$core->db->getRowsCount('scheduler_tasks', "controller = 'content' AND hook = 'publication_notify'")){
        $core->db->query("INSERT INTO `{#}scheduler_tasks` (`title`, `controller`, `hook`, `period`, `is_strict_period`, `date_last_run`, `is_active`, `is_new`) VALUES ('Розсилає сповіщення про закінчення публікації', 'content', 'publication_notify', 1440, 1, DATE_FORMAT(NOW(), '%Y-%m-%d 00:00:05'), 1, 1);");
    }

    save_controller_options(['photos', 'wall', 'comments']);

    if(!$core->db->getRowsCount('controllers', "name = 'wysiwygs'")){
        $core->db->query("INSERT INTO `{#}controllers` (`title`, `name`, `is_enabled`, `options`, `author`, `url`, `version`, `is_backend`) VALUES ('Wysiwyg редактори', 'wysiwygs', 1, NULL, 'InstantCMS Team', 'https://instantcms.ru', '2.0', 1);");
    }

    $core->db->query("ALTER TABLE `{#}uploaded_files` CHANGE `type` `type` VARCHAR(32) NULL DEFAULT 'file' COMMENT 'Тип файлу';");
